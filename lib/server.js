...
Server.prototype.handleRequest = function(req, res) {
	...
	// respond to request
	zone.resolve(request.getProxyURL(), function(err, stream, paths) {
		...
		res.setHeader('X-Content-Source', paths[paths.length - 1] || 'Unknown');
		self.log.status(200, request, 'success');
		responder.respond(stream, request, response);
	});
};
